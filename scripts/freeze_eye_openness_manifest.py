"""Freeze the eye-openness Stage-B comparison plan + GPU manifest (CPU-only).

Arm #113 (NEW deterministic part eye-openness, registered 2026-08-10 via the
gated propose-dimensions channel, selected by the epsilon-greedy EXPLORE slot
at selection_progress 32). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind=\"eye-openness\"` and the
exact frozen generation settings from the arm-#4 manifest, so the only
changed axis is the declared specialist (deterministic max lid-aperture /
interpupillary-distance band from pose2 GOLIATH-308 eyelid-line keypoints).

Band calibration (measured 2026-08-10 frozen 24-item cohort probe): 23/24
measured, distribution open 13 / lidded 10 / closed 0-1 (max_share 0.565,
no band >= 75%), ratio min 0.041 / p25 0.061 / median 0.129 / p75 0.189 /
max 0.460; the closed band fires on the keypoint-dropped closed-signature
(0 measurable items exhibit it; the whole-face keypoint-drop item 08v25q...
abstains). Cross-module corroboration: the three eye-color-abstained items
(0yo0gx..., 0mel7e..., 08v25q...) are exactly the three lowest apertures.
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-eye-openness-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-eye-openness-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-eye-openness-v1")
JOB_ID = "stratum-stage-b-eye-openness-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="eye-openness")
    expected_id = "stage-b-first500-eye-openness-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-eye-openness-v1.json",
