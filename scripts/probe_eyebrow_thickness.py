"""CPU capability + band-calibration probe for eyebrow-thickness (arm #127).

NEW evidence part `eyebrow-thickness` (reuses the already-qualified MediaPipe
...
probe on the frozen 24-item cohort: per-item brow_thickness (mean per-side
brow vertical extent / eye width, sides measured via the facemesh brow-arc
upper/lower chains) so band floors are calibrated from the real cohort
...
Output: /mnt/nas-ai-models/research/stratum/eyebrow-thickness-calibration-probe.json
"""
...
from research_harness.eyebrow_thickness import (  # noqa: E402
    PAIR_EXTENT_PLAUSIBLE,
    THICK_MIN,
    THIN_MAX,
...
OUT = "/mnt/nas-ai-models/research/stratum/eyebrow-thickness-calibration-probe.json"
...
def _synthetic(model_path: str) -> dict:
    ...
    try:
        res = compute_eyebrow_thickness(seg2, blank, model_asset_path=model_path)
    ...
    return {
        ...
        "render": render_eyebrow_thickness(res),
    }
...
def main() -> int:
    ...
    for item in manifest["items"]:
        ...
        try:
            ...
            r = compute_eyebrow_thickness(seg2, rgb, model_asset_path=MODEL)
        except Exception as exc:  # noqa: BLE001
            r = {"abstained": True, "abstention_reason": f"error: {exc!r}"}
        r["image_id"] = image_id
        rows.append(r)
        print(
            f"{image_id[:12]}  band={str(r.get('eyebrow_thickness_band')):<8} "
            f"thickness={r.get('brow_thickness')} "
            f"{'ABSTAIN: ' + str(r.get('abstention_reason')) if r.get('abstained') else ''}",
            flush=True,
        )

    measured = [r for r in rows if not r.get("abstained") and r.get("brow_thickness") is not None]
    vals = sorted(float(r["brow_thickness"]) for r in measured)

    summary = {
        "items": len(rows),
        "n_measured": len(measured),
        "n_abstained": sum(1 for r in rows if r.get("abstained")),
        "plausible_band": list(PAIR_EXTENT_PLAUSIBLE),
        "brow_thickness": {
    ...
    cuts = [
        ...
        (0.10, 0.22),
        (THIN_MAX, THICK_MIN),  # current module constants (provisional)
        (0.12, 0.28),
    ]
    for ci, (cn, cx) in enumerate(cuts):
        wb = Counter()
        for v in vals:
            wb["thin" if v < cn else ("thick" if v > cx else "medium")] += 1
    ...
    wb = Counter()
    for v in vals:
        wb["thin" if v < THIN_MAX else ("thick" if v > THICK_MIN else "medium")] += 1
    n = len(vals)
    summary["final_band"] = {
        "thin_max": THIN_MAX, "thick_min": THICK_MIN, "bands": dict(wb),
