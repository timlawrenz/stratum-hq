#!/usr/bin/env python
"""Freeze the clothing Stage-B comparison plan + GPU manifest (CPU-only).

Arm #29. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="clothing"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (DOME-29 seg2 garment classes + dominant
colors from source pixels instead of pose2+seg2 geometry). No model is
invoked; no corpus write; outputs land only in the noncanonical research tree
under the experiment repo.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-clothing-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-clothing-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-clothing-v1")
JOB_ID = "stratum-stage-b-clothing-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="clothing")
    expected_id = "stage-b-first500-clothing-v1"
