#!/usr/bin/env python
"""Freeze the gaze/head-orientation Stage-B comparison plan + GPU manifest.

Arm #68. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="gaze-head-orientation"` and the exact frozen
generation settings from the arm-#4 manifest, so the only changed axis is the
declared specialist (scale-invariant camera-relative head-orientation bands
from a locally-run open-weight MediaPipe FaceLandmarker 478-point mesh, the
SAME validated asset as arm #60 face-geometry, reused for a NEW evidence part
— the relational camera-interaction axis). The mesh consumes the already-
decoded full-frame source RGB + the seg2 Face_Neck mask (union crop fallback,
read-only validation data); no derived artifact is mutated. No model is
invoked; no corpus write; outputs land only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-gaze-head-orientation-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-gaze-head-orientation-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-gaze-head-orientation-v1")
JOB_ID = "stratum-stage-b-gaze-head-orientation-v1"
...
# Frozen open-weight MediaPipe FaceLandmarker asset (arm #68, reuse arm #60).
MODEL_PATH = Path("/mnt/nas-ai-models/research/stratum/models/face-geometry/face_landmarker.task")
...
def main() -> int:
    ...
    if not MODEL_PATH.exists():
        print(f"FAIL model asset missing: {MODEL_PATH}", file=sys.stderr)
        return 2
    model_sha = _sha256(MODEL_PATH)

    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="gaze-head-orientation")
    expected_id = "stage-b-first500-gaze-head-orientation-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-gaze-head-orientation-v1.json",
