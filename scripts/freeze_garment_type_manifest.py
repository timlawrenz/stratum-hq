"""Freeze the garment-type Stage-B comparison plan + GPU manifest (CPU-only).

Arm #97 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="garment-type"` and the exact frozen generation settings from
the arm-#4 manifest, so the only changed axis is the declared specialist
(deterministic upper/lower garment silhouette split from seg2 DOME-29 clothing
classes — CPU, no new model). Band calibration (measured 2026-08-08 frozen
24-item cohort probe): 24/24 measured, distribution upper-lower-covered 7 /
upper-only 3 / lower-only 4 / skin-dominant 10 (max_share 0.42, no band >= 75%).
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-garment-type-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-garment-type-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-garment-type-v1")
JOB_ID = "stratum-stage-b-garment-type-v1"
...
def main() -> int:
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-garment-type-v1.json",
