#!/usr/bin/env python
"""Freeze the lighting Stage-B comparison plan + GPU manifest (CPU-only).

Arm #33. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="lighting"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (normal2 direction statistics + source
luminance/histogram/dynamic-range instead of pose2+seg2 geometry). Lighting
binds `seg2.npy + normal2.npy` as its evidence inputs. No model is invoked; no
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="lighting")
    expected_id = "stage-b-first500-lighting-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-lighting-v1.json",
