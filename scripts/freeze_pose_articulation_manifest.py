#!/usr/bin/env python
"""Freeze the pose-articulation Stage-B comparison plan + GPU manifest (CPU-only).

Arm #62. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="pose-articulation"` and the exact frozen
generation settings from the already-approved arm-#4 manifest, so the only
changed axis is the declared evidence specialist (scale-invariant kinematic
articulation: joint flexion angles, torso/pelvis orientation, stance /
contrapposto, limb-overlap structure from pose2 GOLIATH-308 keypoints + seg2
DOME-29 masks instead of pose2+seg2 geometry). Pose-articulation binds
`pose2.npy` + `seg2.npy` as its evidence inputs. No model is invoked; no
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-pose-articulation-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-pose-articulation-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-pose-articulation-v1")
JOB_ID = "stratum-stage-b-pose-articulation-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="pose-articulation")
    expected_id = "stage-b-first500-pose-articulation-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-pose-articulation-v1.json",
