"""Freeze the background-color Stage-B comparison plan + GPU manifest (CPU-only).

Arm #126 (NEW evidence part background-color, deterministic dominant hue-family
band warm/cool/neutral over the seg2 DOME-29 Background region from the decoded
source RGB; no new model; registered 2026-08-11 via the gated propose-dimensions
channel, selected by the SELECTOR EXPLOIT slot). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="background-color"` and the exact frozen generation settings from
...
NON-degenerate), coverage floor 8/24 MET, 0 abstentions.
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-background-color-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-background-color-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-background-color-v1")
JOB_ID = "stratum-stage-b-background-color-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="background-color")
    expected_id = "stage-b-first500-background-color-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-background-color-v1.json",
