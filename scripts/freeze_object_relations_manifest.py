#!/usr/bin/env python
"""Freeze the object-relations Stage-B comparison plan + GPU manifest (CPU-only).

Arm #61 (NEW model class). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind="object-relations"` and the
exact frozen generation settings from the arm-#4 manifest, so the only changed
axis is the declared specialist (scale-invariant object-presence count/placement
bands + canonical class list from a locally-run open-weight Grounding DINO
text-grounded detector over a frozen closed vocabulary). Object-relations binds
`seg2.npy` as its evidence input (pose2 stays a validation-only read). The
detector (IDEA-Research grounding-dino-base, model.safetensors sha256
5548f844...) is a process-local open-weight model bound by its sha256 in the
evidence declaration. No model is invoked; no corpus write; outputs land only
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-object-relations-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-object-relations-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-object-relations-v1")
JOB_ID = "stratum-stage-b-object-relations-v1"
...
# Frozen open-weight Grounding DINO asset directory (arm #61).
MODEL_DIR = Path("/mnt/nas-ai-models/research/stratum/models/object-relations")
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="object-relations")
    expected_id = "stage-b-first500-object-relations-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-object-relations-v1.json",
