"""Freeze the jewelry Stage-B comparison plan + GPU manifest (CPU-only).

Arm #112 (NEW evidence part jewelry + NEW MODEL CLASS CLIP ViT-L/14 zero-shot,
registered 2026-08-10 via the gated propose-dimensions channel). Reuses the
tested `build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="jewelry"` and the exact frozen generation settings from the
arm-#4 manifest, so the only changed axis is the declared specialist
(scale-invariant merged jewelry-presence band — earrings OR necklace — from
the local open-weight CLIP ViT-L/14 zero-shot classifier over the seg2
Face_Neck crop; DOME-29 has NO Ear/Neck class, Face_Neck is the declared
honest proxy; model openai/clip-vit-large-patch14 sha256 a2bf730a0c..., the
same staged checkpoint as scene-category #69).

Capability probe (measured 2026-08-10 frozen 24-item cohort,
/mnt/nas-ai-models/research/stratum/jewelry-calibration-probe.json):
23/24 measured (1 honest abstention — Face_Neck below the 200px floor),
earrings sub-band 6/17 (max_share 0.739 < 0.75 NON-degenerate), necklace
sub-band 5/18 (max_share 0.783 >= 0.75 DEGENERATE -> silenced to payload-only
per the arm-#74 precedent), VERBALIZED merged band 8/23 (max_share 0.652
NON-degenerate), coverage floor 8/24 MET (pre-registered >=8/24 with any
jewelry so the axis isn't all-null).
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-jewelry-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-jewelry-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-jewelry-v1")
JOB_ID = "stratum-stage-b-jewelry-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="jewelry")
    expected_id = "stage-b-first500-jewelry-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-jewelry-v1.json",
