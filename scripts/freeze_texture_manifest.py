#!/usr/bin/env python
"""Freeze the texture Stage-B comparison plan + GPU manifest (CPU-only).

Arm #35. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="texture"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (per-region-class edge/gradient texture
statistics from seg2 masks + source pixels instead of pose2+seg2 geometry).
Texture binds `seg2.npy` as its evidence input. No model is invoked; no corpus
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-texture-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-texture-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-texture-v1")
JOB_ID = "stratum-stage-b-texture-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="texture")
    expected_id = "stage-b-first500-texture-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-texture-v1.json",
