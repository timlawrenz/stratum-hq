"""Freeze the affordance-contact Stage-B comparison plan + GPU manifest.

Arm #76 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="affordance-contact"` and the exact frozen generation settings
from the arm-#4 manifest, so the only changed axis is the declared specialist
(scale-invariant subject self-contact / affordance bands from pose2 GOLIATH-308
keypoints + seg2 DOME-29 masks — CPU, no new model). After the 2026-08-07
calibration probe, verbalized bands: hand-own-body contact count (0/1/2),
hand-elevation count, and the grounded binary. Subject-to-EXTERNAL-object
contact is NOT claimed (seg2 has no object classes). No model is invoked; no
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-affordance-contact-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-affordance-contact-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-affordance-contact-v1")
JOB_ID = "stratum-stage-b-affordance-contact-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="affordance-contact")
    expected_id = "stage-b-first500-affordance-contact-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-affordance-contact-v1.json",
