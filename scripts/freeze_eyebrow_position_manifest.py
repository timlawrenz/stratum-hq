"""Freeze the eyebrow-position Stage-B comparison plan + GPU manifest (CPU-only).

Arm #111 (NEW evidence part eyebrow-position, registered 2026-08-10 via the
gated propose-dimensions channel, exploitative selector selection). Reuses the
tested `build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="eyebrow-position"` and the exact frozen generation settings
from the arm-#4 manifest, so the only changed axis is the declared specialist
(deterministic mean brow-arch elevation above the eye line / ipsilateral eye
width from the local MediaPipe FaceLandmarker mesh — normal: the arm-#60
model, face_landmarker.task sha256 64184e229b...).

Band calibration (measured 2026-08-10 frozen 24-item cohort probe): 21/24
measured, distribution neutral 13 / raised 5 / furrowed-low 3 (max_share
0.619, no band >= 75%), arch_elevation min 0.017 / p25 0.672 / median 0.745 /
p75 0.946 / max 1.691; the 3 abstains (07hyx5..., 0mel7e..., 08v25q...) are
the same turned-head / no-face-region items the facemesh cohort already
reports. The inner-brow (furrow) composite rule was rejected on probe review
as threshold-fitting; inner_brow_elevation stays a corroborating payload
signal.
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-eyebrow-position-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-eyebrow-position-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-eyebrow-position-v1")
JOB_ID = "stratum-stage-b-eyebrow-position-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="eyebrow-position")
    expected_id = "stage-b-first500-eyebrow-position-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-eyebrow-position-v1.json",
