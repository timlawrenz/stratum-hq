"""Freeze the environment-clearance Stage-B comparison plan + GPU manifest.

Arm #85 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="environment-clearance"` and the exact frozen generation
settings from the arm-#4 manifest, so the only changed axis is the declared
specialist (subject-to-backdrop negative-space band from seg2 — CPU, no new
model). After the 2026-08-08 calibration probe (band-degeneracy recovery:
re-cut from the median-of-4-axes, degenerate at 86% tight, to left/right
horizontal negative space), the verbalized band is tight / moderate / spacious
(10/9/3, max_share 0.45); 22/24 measured, 2 honest abstentions (full-bleed
subjects with zero Background clearance). No model is invoked; no corpus
write; outputs land only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-environment-clearance-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-environment-clearance-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-environment-clearance-v1")
JOB_ID = "stratum-stage-b-environment-clearance-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="environment-clearance")
    expected_id = "stage-b-first500-environment-clearance-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-environment-clearance-v1.json",
