#!/usr/bin/env python
"""Freeze the camera-viewing-angle Stage-B comparison plan + GPU manifest.

Arm #74 (NEW deterministic evidence part). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="camera-viewing-angle"` and the exact frozen generation settings
from the arm-#4 manifest, so the only changed axis is the declared specialist
(scale-invariant camera-relative framing bands from the seg2 subject mask +
full-frame geometry — CPU, no new model). After the 2026-08-07 calibration
probe, ONLY the headroom band is verbalized (shot-scale and camera-height were
degenerate uniform axes on this portrait-centric cohort and are payload-only).
No model is invoked; no corpus write; outputs land only in the noncanonical
research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-camera-viewing-angle-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-camera-viewing-angle-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-camera-viewing-angle-v1")
JOB_ID = "stratum-stage-b-camera-viewing-angle-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="camera-viewing-angle")
    expected_id = "stage-b-first500-camera-viewing-angle-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-camera-viewing-angle-v1.json",
