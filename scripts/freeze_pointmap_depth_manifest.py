#!/usr/bin/env python
"""Freeze the pointmap-depth Stage-B comparison plan + GPU manifest (CPU-only).

Arm #58. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="pointmap-depth"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis is
the declared evidence specialist (scale-invariant relative-depth ordering from
pointmap.npy CAM-frame cloud + seg2.npy DOME-29 masks instead of pose2+seg2
geometry). Pointmap-depth binds `pointmap.npy` + `seg2.npy` as its evidence
inputs (pose2 stays a validation-only read). No model is invoked; no corpus
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-pointmap-depth-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-pointmap-depth-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-pointmap-depth-v1")
JOB_ID = "stratum-stage-b-pointmap-depth-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="pointmap-depth")
    expected_id = "stage-b-first500-pointmap-depth-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-pointmap-depth-v1.json",
