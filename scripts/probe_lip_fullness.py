"""CPU capability + band-calibration probe for lip-fullness (arm #122).

NEW evidence part `lip-fullness` (reuses the already-qualified MediaPipe
FaceLandmarker 478-point mesh — the same model as face-geometry #60, gaze-head
#68, eyebrow-position #111, nose-geometry #121; no new model). Band-calibration
probe on the frozen 24-item cohort: report the per-item lip_fullness_ratio
(vermilion height / mouth width) distribution so band floors are calibrated
...
tree (identical bytes, SHA-verified by the frozen manifest) when the approved
entry was purged — the hold-#132 (2 of 24 sources purged) documented pattern,
mirroring probe_nose_geometry.py.

Read-only, CPU, no new model, no corpus write.
Output: /mnt/nas-ai-models/research/stratum/lip-fullness-calibration-probe.json
"""
...
from research_harness.lip_fullness import (  # noqa: E402
    FULL_MIN,
    LIP_RATIO_PLAUSIBLE,
    THIN_MAX,
...
OUT = "/mnt/nas-ai-models/research/stratum/lip-fullness-calibration-probe.json"
...
def _synthetic(model_path: str) -> dict:
    ...
    try:
        res = compute_lip_fullness(seg2, blank, model_asset_path=model_path)
    ...
    return {
        ...
        "render": render_lip_fullness(res),
    }
...
def main() -> int:
    ...
    for item in manifest["items"]:
        ...
        try:
            ...
            r = compute_lip_fullness(seg2, rgb, model_asset_path=MODEL)
        except Exception as exc:  # noqa: BLE001
            r = {"abstained": True, "abstention_reason": f"error: {exc!r}"}
        r["image_id"] = image_id
        rows.append(r)
        print(
            f"{image_id[:12]}  band={str(r.get('lip_fullness_band')):<8} "
            f"ratio={r.get('lip_fullness_ratio')} "
            f"{'ABSTAIN: ' + str(r.get('abstention_reason')) if r.get('abstained') else ''}",
            flush=True,
        )

    measured = [r for r in rows if not r.get("abstained") and r.get("lip_fullness_ratio") is not None]
    vals = sorted(float(r["lip_fullness_ratio"]) for r in measured)

    summary = {
        "items": len(rows),
        "n_measured": len(measured),
        "n_abstained": sum(1 for r in rows if r.get("abstained")),
        "plausible_band": list(LIP_RATIO_PLAUSIBLE),
        "lip_fullness_ratio": {
    ...
    # Candidate 3-way cuts: report the degeneracy consequence of several cuts so
    # the calibration is transparent (band-degeneracy rule enforced).
    band_candidates = []
    cuts = [
        (round(float(np.percentile(vals, 33.3)), 3), round(float(np.percentile(vals, 66.6)), 3)),
        (0.28, 0.45),
        (THIN_MAX, FULL_MIN),  # current module constants (provisional)
        (0.22, 0.52),
    ]
    for ci, (cn, cx) in enumerate(cuts):
        wb = Counter()
        for v in vals:
            wb["thin" if v < cn else ("full" if v > cx else "medium")] += 1
    ...
    wb = Counter()
    for v in vals:
        wb["thin" if v < THIN_MAX else ("full" if v > FULL_MIN else "medium")] += 1
    n = len(vals)
    summary["final_band"] = {
        "thin_max": THIN_MAX, "full_min": FULL_MIN, "bands": dict(wb),
