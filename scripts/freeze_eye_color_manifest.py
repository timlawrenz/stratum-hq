"""Freeze the eye-color Stage-B comparison plan + GPU manifest.

Arm #80 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="eye-color"` and the exact frozen generation settings from the
arm-#4 manifest, so the only changed axis is the declared specialist
(deterministic iris-hue band from pose2 iris/pupil keypoints + source RGB —
CPU, no new model). After the 2026-08-08 calibration probe (re-cut classifier),
the verbalized band is brown / dark / blue / green-hazel / gray (15/5/1,
max_share 0.71); 21/24 measured, 3 honest abstentions (eyes
closed/cropped/occluded). No model is invoked; no corpus write; outputs land
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-eye-color-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-eye-color-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-eye-color-v1")
JOB_ID = "stratum-stage-b-eye-color-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="eye-color")
    expected_id = "stage-b-first500-eye-color-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-eye-color-v1.json",
