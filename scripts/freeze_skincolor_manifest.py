#!/usr/bin/env python
"""Freeze the skin-color Stage-B comparison plan + GPU manifest (CPU-only).

Arm #31. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="skin-color"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (DOME-29 exposed-skin regions + dominant
skin tone from source pixels instead of pose2+seg2 geometry). No model is
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-skin-color-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-skin-color-v1.json"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="skin-color")
    expected_id = "stage-b-first500-skin-color-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-skin-color-v1.json",
