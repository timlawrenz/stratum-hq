"""Freeze the body-configuration Stage-B comparison plan + GPU manifest.

Arm #83 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="body-configuration"` and the exact frozen generation settings
from the arm-#4 manifest, so the only changed axis is the declared specialist
(whole-body posture class from pose2 GOLIATH-308 keypoints — CPU, no new
model). After the 2026-08-07 calibration probe, the verbalized band is the
coarse posture class (standing / seated / reclined); 14/24 measured, max_share
0.50, honest abstentions for sparse/ambiguous skeletons. No model is invoked;
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-body-configuration-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-body-configuration-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-body-configuration-v1")
JOB_ID = "stratum-stage-b-body-configuration-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="body-configuration")
    expected_id = "stage-b-first500-body-configuration-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-body-configuration-v1.json",
