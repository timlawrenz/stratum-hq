#!/usr/bin/env python
"""Freeze the body-type Stage-B comparison plan + GPU manifest (CPU-only).

Arm #32. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="body-type"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (pose2 proportions instead of pose2+seg2
geometry). No model is invoked; no corpus write; outputs land only in the
noncanonical research tree under the experiment repo.
"""
...
def main() -> int:
    ...
    expected_id = "stage-b-first500-bodytype-v1"
    ...
    # Mirror the arm-#4 manifest shape. Compare: same candidate + generation,
    # different plan fingerprint; distinct job_id + output root; identical
    # launcher/runner hashes only AFTER the committed code matches (see note).
    manifest = {
        "schema_version": 1,
        "approved_issue": 18,
        "authorization": {
