#!/usr/bin/env python
"""Freeze the scene-category Stage-B comparison plan + GPU manifest (CPU-only).

Arm #69 (NEW model class). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind="scene-category"` and the
exact frozen generation settings from the arm-#4 manifest, so the only changed
axis is the declared specialist (semantic scene-category label from a locally-
run open-weight CLIP ViT-L/14 zero-shot classifier over the full-frame source
with a frozen closed scene-category set at a calibrated confidence floor).
Scene-category binds NO derived evidence artifact (the CLIP model consumes only
the already-decoded full-frame source RGB, SHA-bound via the item's
source_sha256); seg2/pose2 stay validation-only reads for the exactly-one-
subject invariant. The CLIP model (openai/clip-vit-large-patch14,
model.safetensors sha256 a2bf730a...) is a process-local open-weight model
bound by its sha256 in the evidence declaration. No model is invoked; no
corpus write; outputs land only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-scene-category-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-scene-category-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-scene-category-v1")
JOB_ID = "stratum-stage-b-scene-category-v1"
...
# Frozen open-weight CLIP ViT-L/14 asset directory (arm #69).
MODEL_DIR = Path("/mnt/nas-ai-models/research/stratum/models/scene-category")
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="scene-category")
    expected_id = "stage-b-first500-scene-category-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-scene-category-v1.json",
