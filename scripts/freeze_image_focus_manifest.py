#!/usr/bin/env python
"""Freeze the image-focus Stage-B comparison plan + GPU manifest.

Arm #75 (NEW deterministic evidence part). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="image-focus"` and the exact frozen generation settings from
the arm-#4 manifest, so the only changed axis is the declared specialist
(scale-invariant subject/background acutance bands + DOF ratio from the
already-decoded source RGB and the seg2 region split — CPU, no new model).
After the 2026-08-07 two-pass calibration probe, the DOF ratio (bg/subject
interior median acutance, cuts 0.45/0.80: 9/8/7) and the subject-vs-frame
ratio (cuts 0.9/1.6: 3/12/9) are the verbalized bands; the subject-vs-frame-
P99 discriminator was rejected (24/24 degenerate) and a flat-background
abstention guard stays (bg_p99 < 4.0: nothing to resolve as sharp or
blurred). No model is invoked; no corpus write; outputs land only in the
noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-image-focus-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-image-focus-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-image-focus-v1")
JOB_ID = "stratum-stage-b-image-focus-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="image-focus")
    expected_id = "stage-b-first500-image-focus-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-image-focus-v1.json",
