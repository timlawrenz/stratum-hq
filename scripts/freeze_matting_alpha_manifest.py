#!/usr/bin/env python
"""Freeze the matting-alpha Stage-B comparison plan + GPU manifest (CPU-only).

Arm #59. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="matting-alpha"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis is
the declared evidence specialist (scale-invariant alpha-fidelity from matting.npy
alpha matte + seg2.npy DOME-29 masks instead of pose2+seg2 geometry).
Matting-alpha binds `matting.npy` + `seg2.npy` as its evidence inputs (pose2
stays a validation-only read). No model is invoked; no corpus write; outputs
land only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-matting-alpha-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-matting-alpha-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-matting-alpha-v1")
JOB_ID = "stratum-stage-b-matting-alpha-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="matting-alpha")
    expected_id = "stage-b-first500-matting-alpha-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-matting-alpha-v1.json",
