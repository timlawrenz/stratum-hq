#!/usr/bin/env python
"""Freeze the setting Stage-B comparison plan + GPU manifest (CPU-only).

Arm #34. Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="setting"` and the exact frozen generation
settings from the already-approved arm-#4 manifest, so the only changed axis
is the declared evidence specialist (seg2 Background-class coverage + source
palette stats instead of pose2+seg2 geometry). Setting binds `seg2.npy` as its
evidence input. No model is invoked; no corpus write; outputs land only in the
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-setting-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-setting-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-setting-v1")
JOB_ID = "stratum-stage-b-setting-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="setting")
    expected_id = "stage-b-first500-setting-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-setting-v1.json",
