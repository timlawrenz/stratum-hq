#!/usr/bin/env python
"""Freeze the apparent-age Stage-B comparison plan + GPU manifest (CPU-only).

Arm #73 (NEW model class). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind="apparent-age"` and the
exact frozen generation settings from the arm-#4 manifest, so the only changed
axis is the declared specialist (scale-invariant apparent-age band from a local
open-weight MiVOLO-V2 age+gender transformer). Apparent-age binds `seg2.npy`
as its evidence input (pose2 stays a validation-only read; the source RGB is
already SHA-bound per item). The MiVOLO-V2 model (model.safetensors + vendored
mivolo package) is a process-local open-weight model bound by its sha256 in
the evidence declaration. No model is invoked at freeze time; no corpus write;
outputs land only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-apparent-age-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-apparent-age-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-apparent-age-v1")
JOB_ID = "stratum-stage-b-apparent-age-v1"
...
# Frozen open-weight MiVOLO-V2 asset directory (arm #73).
MODEL_DIR = Path("/mnt/nas-ai-models/research/stratum/models/apparent-age")
...
def main() -> int:
    ...
    if not model_file.exists():
        print(f"FAIL model asset missing: {model_file}", file=sys.stderr)
        return 2
    model_sha = _sha256(model_file)

    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="apparent-age")
    expected_id = "stage-b-first500-apparent-age-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-apparent-age-v1.json",
