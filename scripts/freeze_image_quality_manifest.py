#!/usr/bin/env python
"""Freeze the image-quality Stage-B comparison plan + GPU manifest (CPU-only).

Arm #95 (NEW model class). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind="image-quality"` and the
exact frozen generation settings from the arm-#4 manifest, so the only changed
axis is the declared specialist (no-reference perceptual-quality band from a
locally-run open-weight CLIP ViT-L/14 implementing the zero-shot CLIP-IQA
scoring method, Wang et al. AAAI 2023, over the full-frame source at
calibrated band floors). Image-quality binds NO derived evidence artifact (the
CLIP model consumes only the already-decoded full-frame source RGB, SHA-bound
via the item's source_sha256); seg2/pose2 stay validation-only reads for the
exactly-one-subject invariant. The CLIP model (openai/clip-vit-large-patch14,
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-image-quality-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-image-quality-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-image-quality-v1")
JOB_ID = "stratum-stage-b-image-quality-v1"
...
# Frozen open-weight CLIP ViT-L/14 asset directory (arm #95).
...
def main() -> int:
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-image-quality-v1.json",
