"""Freeze the bangs-forehead Stage-B comparison plan + GPU manifest (CPU-only).

Arm #110 (NEW deterministic part bangs-forehead, registered 2026-08-10 via the
gated propose-dimensions channel — brainstorm-new-data). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="bangs-forehead"` and the exact frozen generation settings from
the arm-#4 manifest, so the only changed axis is the declared specialist
(deterministic seg2 Hair over the eye-line-anchored forehead band + pose2
GOLIATH-308 eye line). Band calibration (measured 2026-08-10 frozen 24-item
cohort probe): 22/24 measured, distribution bangs 4 / partial-fringe 9 /
swept-back 9 (max_share 0.409, no band >= 75%), coverage min 0.0 / median
0.177 / max 0.483. No corpus write; outputs land only in the noncanonical
research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-bangs-forehead-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-bangs-forehead-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-bangs-forehead-v1")
JOB_ID = "stratum-stage-b-bangs-forehead-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="bangs-forehead")
    expected_id = "stage-b-first500-bangs-forehead-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-bangs-forehead-v1.json",
