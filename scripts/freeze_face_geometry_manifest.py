#!/usr/bin/env python
"""Freeze the face-geometry Stage-B comparison plan + GPU manifest (CPU-only).

Arm #60 (NEW model class). Reuses the tested `build_stage_b_plan` /
`freeze_stage_b_plan` machinery with `evidence_kind="face-geometry"` and the
exact frozen generation settings from the arm-#4 manifest, so the only changed
axis is the declared specialist (scale-invariant facial-geometry bands from a
local open-weight MediaPipe FaceLandmarker 478-point mesh over the full frame /
seg2 Face_Neck crop, union detection policy). Face-geometry binds `seg2.npy` as
its evidence input (pose2 stays a validation-only read). The landmarker model
(face_landmarker.task) is a process-local open-weight model bound by its sha256
in the evidence declaration. No model is invoked; no corpus write; outputs land
only in the noncanonical research tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-face-geometry-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-face-geometry-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-face-geometry-v1")
JOB_ID = "stratum-stage-b-face-geometry-v1"
...
# Frozen open-weight MediaPipe FaceLandmarker asset (arm #60).
MODEL_ASSET = Path("/mnt/nas-ai-models/research/stratum/models/face-geometry/face_landmarker.task")
...
def main() -> int:
    ...
    if not MODEL_ASSET.exists():
        print(f"FAIL model asset missing: {MODEL_ASSET}", file=sys.stderr)
        return 2
    model_sha = _sha256(MODEL_ASSET)

    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="face-geometry")
    expected_id = "stage-b-first500-face-geometry-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-face-geometry-v1.json",
