#!/usr/bin/env python
"""Freeze the garment-type REVISION-2 Stage-B comparison plan + GPU manifest.

Arm #97, strike-1 retry (2026-08-10). The strike-1 round-trip
(stage-b-garment-type-v1) was harness-computed NOT_BETTER: support ratio
improved 0.4624 -> 0.8564 but the sign test was not significant (p=0.10502,
15/23 paired positives). Root cause (from the independent review's
contradictions): the caption model FABRICATED garments the evidence marks
absent — "low-rise pink bottoms" on skin-dominant items, "pale blue
button-down shirt" on lower-only items, "black one-piece swimsuit" on
upper-only items. The measurement itself is correct and non-degenerate
(probe 7/3/10/4, max_share 0.42); the evidence->caption channel dropped the
measured ABSENCE signal (upper/lower_garment_present booleans were payload-only).

Revision-2 is an aspect-level recovery of the rendering, exactly the arm #95
band-degeneracy-recovery pattern: `_serialize_garment_type` (stage_b.py) and
`render_garment_type` (garment_type.py) now verbalize the measured absence
explicitly ("NO lower-body garment present", "NO upper-body garment present
AND NO lower-body garment present") and add a do-NOT-fabricate instruction
("do NOT describe any lower-body garment" / "do NOT describe any garment").
The deterministic measurement, bands, floors, cohort, model, and review
protocol are UNCHANGED — only the rendered evidence text is more faithful.

This is a NEW round-trip with a distinct plan/manifest/output-root/job-id
(revision-2), additive and non-overwriting of the revision-1 artifacts.
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-garment-type-v2.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-garment-type-v2.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-garment-type-v2")
JOB_ID = "stratum-stage-b-garment-type-v2"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="garment-type")
    expected_id = "stage-b-first500-garment-type-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-garment-type-v2.json",
