"""Freeze the hairstyle Stage-B comparison plan + GPU manifest.

Arm #82 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="hairstyle"` and the exact frozen generation settings from the
arm-#4 manifest, so the only changed axis is the declared specialist
(hair-length + hair-arrangement bands from seg2 Hair + pose2 shoulders — CPU,
no new model). After the 2026-08-08 calibration probe, the verbalized bands are
the scale-invariant length band (long 13 / shoulder-length 5 / short 4) and
arrangement band (down 15 / kept-up 7); both below the 75% degeneracy line;
22/24 measured with honest abstentions for unreliable shoulder/neck. No model
is invoked; no corpus write; outputs land only in the noncanonical research
tree.
"""
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-hairstyle-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-hairstyle-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-hairstyle-v1")
JOB_ID = "stratum-stage-b-hairstyle-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="hairstyle")
    expected_id = "stage-b-first500-hairstyle-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-hairstyle-v1.json",
