"""Freeze the face-visibility Stage-B comparison plan + GPU manifest.

Arm #84 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="face-visibility"` and the exact frozen generation settings from
the arm-#4 manifest, so the only changed axis is the declared specialist
(face-prominence band from seg2 Face_Neck + Hair — CPU, no new model). After
the 2026-08-08 calibration probe (band-degeneracy recovery: re-cut from the
structurally-impossible pixel-overlap occlusion measure to the face-to-hair
prominence ratio with a proportional local-head-window margin), the verbalized
band is clearly-visible / partially-framed / hair-dominant (12/7/4, max_share
0.52); 23/24 measured, 1 honest abstention (no Face_Neck region). No model is
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-face-visibility-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-face-visibility-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-face-visibility-v1")
JOB_ID = "stratum-stage-b-face-visibility-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="face-visibility")
    expected_id = "stage-b-first500-face-visibility-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-face-visibility-v1.json",
