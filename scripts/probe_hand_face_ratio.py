"""CPU capability + band-calibration probe for hand-face-ratio (arm #124).

NEW relational evidence part `hand-face-ratio` (reuses the already-qualified
MediaPipe FaceLandmarker 478-point mesh from face-geometry #60 AND the MediaPipe
HandLandmarker from hand-gesture #109 — no new model class). Band-calibration
probe on the frozen 24-item cohort: per-item hand_face_ratio (mean palm-length
/ face-width over measured hands) so band floors are calibrated from the real
cohort (band-degeneracy rule: a single band >= 75% of measured items means the
axis does not discriminate).
...
Output: /mnt/nas-ai-models/research/stratum/hand-face-ratio-calibration-probe.json
"""
...
from research_harness.hand_face_ratio import (  # noqa: E402
    HAND_FACE_PLAUSIBLE,
    LARGE_MIN,
    SMALL_MAX,
...
FACE_MODEL = "/mnt/nas-ai-models/research/stratum/models/face-geometry/face_landmarker.task"
HAND_MODEL = "/mnt/nas-ai-models/research/stratum/models/hand-gesture/hand_landmarker.task"
...
OUT = "/mnt/nas-ai-models/research/stratum/hand-face-ratio-calibration-probe.json"


def _synthetic(face_model: str, hand_model: str) -> dict:
    ...
    try:
        res = compute_hand_face_ratio(
            blank, seg2,
            face_model_asset_path=face_model,
            hand_model_asset_path=hand_model,
        )
    ...
    return {
        ...
        "render": render_hand_face_ratio(res),
    }
...
def main() -> int:
    ...
    synthetic = _synthetic(FACE_MODEL, HAND_MODEL)
    ...
    for item in manifest["items"]:
        ...
        rows.append(r)
        print(
            f"{image_id[:12]}  band={str(r.get('hand_size_band')):<8} "
            f"ratio={r.get('hand_face_ratio')} "
            f"{'ABSTAIN: ' + str(r.get('abstention_reason')) if r.get('abstained') else ''}",
            flush=True,
        )

    measured = [r for r in rows if not r.get("abstained") and r.get("hand_face_ratio") is not None]
    vals = sorted(float(r["hand_face_ratio"]) for r in measured)

    summary = {
        "items": len(rows),
        "n_measured": len(measured),
        "n_abstained": sum(1 for r in rows if r.get("abstained")),
        "plausible_band": list(HAND_FACE_PLAUSIBLE),
        "hand_face_ratio": {
    ...
    cuts = [
        ...
        (0.55, 0.90),
        (SMALL_MAX, LARGE_MIN),  # current module constants (provisional)
        (0.60, 1.10),
    ]
    for ci, (cn, cx) in enumerate(cuts):
        wb = Counter()
        for v in vals:
            wb["small" if v < cn else ("large" if v > cx else "typical")] += 1
    ...
    wb = Counter()
    for v in vals:
        wb["small" if v < SMALL_MAX else ("large" if v > LARGE_MIN else "typical")] += 1
    n = len(vals)
    summary["final_band"] = {
        "small_max": SMALL_MAX, "large_min": LARGE_MIN, "bands": dict(wb),
