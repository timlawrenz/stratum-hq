"""Freeze the facial-expression Stage-B comparison plan + GPU manifest.

Arm #81 (NEW deterministic evidence part, no new model). Reuses the tested
`build_stage_b_plan` / `freeze_stage_b_plan` machinery with
`evidence_kind="facial-expression"` and the exact frozen generation settings
from the arm-#4 manifest, so the only changed axis is the declared specialist
(deterministic smile/expression band from pose2 mouth-corner keypoints — CPU,
no new model). After the 2026-08-08 calibration probe (band-degeneracy
recovery: re-cut from the degenerate openness-only 3-band to openness + corner
elevation), the verbalized band is neutral / slight-smile / open-smile
(9/8/2, max_share 0.47); 19/24 measured, 5 honest abstentions (mouth
occluded/low-confidence). No model is invoked; no corpus write; outputs land
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-facial-expression-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-facial-expression-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-facial-expression-v1")
JOB_ID = "stratum-stage-b-facial-expression-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="facial-expression")
    expected_id = "stage-b-first500-facial-expression-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-facial-expression-v1.json",
