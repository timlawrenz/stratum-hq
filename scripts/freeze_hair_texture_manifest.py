"""Freeze the hair-texture Stage-B comparison plan + GPU manifest (CPU-only).

Arm #94 (NEW MODEL CLASS re-scope 2026-08-10 under the open-world sourcing
directive). Reuses the tested `build_stage_b_plan` / `freeze_stage_b_plan`
machinery with `evidence_kind="hair-texture"` and the exact frozen generation
settings from the arm-#4 manifest, so the only changed axis is the declared
specialist (open-weight CLIP ViT-L/14 zero-shot hair-texture classifier over
the seg2 Hair crop — straight/wavy/curly/coily, local CPU). Band calibration
(measured 2026-08-10 frozen 24-item cohort CLIP probe): 24/24 measured,
distribution straight 3 / wavy 13 / coily 6 / curly 2 (max_share 0.542, no
band >= 75%), argmax confidence median 0.632 / min 0.373. No corpus write;
...
PLAN_OUT = ROOT / "research/stage-b-plans/stage-b-hair-texture-v1.json"
MANIFEST_OUT = ROOT / "research/gpu-manifests/stage-b-hair-texture-v1.json"
OUTPUT_ROOT = Path("/mnt/nas-ai-models/research/stratum/stage-b-hair-texture-v1")
JOB_ID = "stratum-stage-b-hair-texture-v1"
...
def main() -> int:
    ...
    plan = freeze_stage_b_plan(program, candidate, SETTINGS, evidence_kind="hair-texture")
    expected_id = "stage-b-first500-hair-texture-v1"
    ...
    manifest = {
        ...
        "execution": {
            ...
            "comparison_plan_relative_path": "research/stage-b-plans/stage-b-hair-texture-v1.json",
