...
**Last updated:** 2026-08-10 (decision boundary RESOLVED — #94 hair-texture re-scoped to CLIP zero-shot NEW MODEL CLASS and **VALIDATED BETTER (round-trip, cycle 27)**; body-volume #96 active via Anny re-scope; brainstorm arms #105/#106/#107 blocked with measured degeneracy; 30 validated + 1 active + 3 blocked + 0 proposals)
**Phase / status:** ACTIVE — empirical Stage-B loop running; **30 validated** (27 goal feeders + goal arm dossier-context4k + reconstruction + hair-texture #94) + **1 active (body-volume #96, Anny SMPL-free re-scope)** + **3 blocked (eyewear #105, waist-shape #106, lip-color #107 — all measured degenerate on the frozen cohort)** + 0 proposals; goal inputs 93.1% validated, `goal_unreachable: false` (floor 4001, `any_max_honest_floor_reached` 24/24). The decision boundary is RESOLVED under the open-world sourcing directive (2026-08-05): model-class re-scopes are autonomous, not human rulings.
...
**Execute the active arm: body-volume #96 (Anny SMPL-free whole-body volume regressor, `research:active`).** The SMPL/SMPL-X license gate is RESOLVED by choosing option (b) per the open-world sourcing directive (2026-08-05): re-scoping to **Anny** (naver/anny; Apache-2.0 code, CC0 MakeHuman assets, scan-free, genuinely SMPL-free — native anny topology, no registration). Next: stage Anny + Multi-HMR locally on owned hardware, run the capability + model-qualification probe on the frozen cohort (coverage floor pre-registered, e.g. >=18/24 plausible whole-body meshes) + band calibration (no band >= 75%), then freeze plan + manifest + GPU round-trip (4090 scheduler-managed) + independent review + `autonomous-tick`. Note: the arm's qualification gate now explicitly uses Anny's native free topology; the optional smplx interop topology is deliberately NOT used. The three blocked brainstorm arms (#105/#106/#107) record small decisions on their issues (downgrade / different cohort / drop) — non-blocking for the loop.
