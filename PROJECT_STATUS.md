...
The source-hashed coverage-balanced subset is frozen. Respect #18: await an owner decision that names an already-installed local aggregator and immutable generation settings, freezes the claim-support known-case/null self-audit and adversarial-review plan, and separately authorizes or denies model invocation and GPU/scheduler action for the exact manifest. The completed output root remains `PENDING_INDEPENDENT_REVIEW`; the newest observer-only findings add two interpretation caveats: the evidence-only contrast is not cleanly isolated in the rendered prompts (evidence+instructions confound), and the input-view-only contrast is declared-and-bound but not input-documented (0/96 records carry a per-image view-content digest). The owner must decide whether to interpret the two affected contrasts with the documented caveats or require a re-run with a data-only evidence renderer and per-image view digests.
