...
**Last updated:** 2026-08-11 09:00Z (background-color #126 ROUND-TRIP BETTER, cycle 36; skin-clarity #125 active, selection_progress 37; 36 validated)

**2026-08-11 — BACKGROUND-COLOR #126 ROUND-TRIP BETTER (harness-computed, cycle 36).** NEW deterministic part (backdrop dominant hue family warm/cool/neutral over the seg2 DOME-29 Background region, per-pixel HSV hue-family share, achromatic gate sat<0.15 with neutral-share floor 0.60; barrel-tested scale-invariant; no new model; same MIN_BG_PX/MIN_BG_COVERAGE gates as setting #34 so both arms read the same region). Non-redundance per selection rationale: scene-category #69 = semantic place, setting #34 = pattern/tone/vibrancy/named-palette (no temperature band), environment-clearance #85 = negative space — hue family is the unbound 'blue backdrop' / 'warm beige wall' axis; lightness deliberately payload-only (setting already grounds light/mid/dark tone). Capability probe PASS (24/24 measured, warm 11 / cool 6 / neutral 7, max_share 0.4583 < 0.75; coverage floor met; 0 abstains). Frozen plan `stage-b-first500-background-color-v1` (96 = 24×4) generated on the 4090 via the scheduler (`stratum-stage-b-background-color-v1`, 96 records) + independent adversarial review (96 rows `-review`). `autonomous-tick` computed **BETTER** — baseline 80 supported / 93 unsupported (ratio 0.4624) → evidence **182 supported / 25 unsupported (ratio 0.8792, Δ+0.4168), 20/24 paired positives, sign-test p=0.000772** (inconclusive:false). Registry: background-color → **validated** (cycle 36), **skin-clarity #125 → active** (selected_via EXPLOIT, EIG 0.12 + novelty +0.15, selection_progress 37). 854 tests + all validations green. Label-sync: #126 research:validated, #125 research:active. PR #130 (branch exp/stage-b-background-color-arm126-20260811).

**2026-08-11 05:35Z — eyebrow-position #111 ROUND-TRIP BETTER (harness-computed, cycle 34)** (below; status-file header updated this cycle: 36 validated / 1 active skin-clarity / 4 blocked / 6 proposals).

**2026-08-11 — EYEBROW-POSITION #111 ROUND-TRIP BETTER (harness-computed, cycle 34).** NEW deterministic part (brow-arch elevation vs the eye-line from the already-validated MediaPipe FaceLandmarker 478-pt mesh, full-frame ∪ seg2 Face_Neck crop, banded neutral / raised / furrowed-low; frontal-only; batch computed in-memory — no corpus write). Band calibration probe PASS (21/24 measured, neutral 13 / raised 5 / furrowed-low 3, max_share 0.619 < 0.75; 3 honest abstains = turned-head/no-face items; inner-brow composite furrow rule rejected on probe review as threshold-fitting — kept payload-only). Frozen plan `stage-b-first500-eyebrow-position-v1` (96 = 24×4) generated on the 4090 (`stratum-stage-b-eyebrow-position-v1`, 96 records) + independent adversarial review (96 rows `-review`). `autonomous-tick` computed **BETTER** — baseline 80 supported / 93 unsupported (ratio 0.4624) → evidence **156 supported / 49 unsupported (ratio 0.761, Δ+0.2985), 17/21 paired positives, sign-test p=0.003599** (inconclusive:false). Registry: eyebrow-position → **validated** (cycle 34), **jewelry #112 → active** (selected_via EXPLOIT, selection_progress 35). 825 tests + all validations green. Label-sync: #111 research:validated, #112 research:active.
**GPU-RESERVATION LESSON (this cycle, blocker resolved):** the 4090's desktop graphics stack (Vivaldi/Chrome renderer ~1.13 GiB + gnome-shell + steam + alacritty) holds a persistent ~1.69 GiB, leaving 22386 MiB free = **21.86 GiB** — while every frozen arm manifest requests **22 GiB**, so `poll --gpu 4090` returned `vram_insufficient` by ~0.14 GiB on a clean idle GPU. Fix applied and verified: re-freeze eyebrow manifest with `requested_vram_gb: 21.0` (gemma3:27b real need ~16.3 GiB Q4_K_M weights + KV; 21.0 keeps ~3 GiB margin and claims fine), plus **review launcher `--vram` override** (default 22 unchanged) so the gemma4:e4b review (real footprint ~3.1 GiB) can be requested at 12 GiB. Future arms on this box should reserve 21.0 GiB (gen) / 12 GiB (review), not 22, or they will stall at claim.
**Phase / status:** ACTIVE — empirical Stage-B loop running; **34 validated** (+ eyebrow-position #111) + **1 active (jewelry #112)** + **4 blocked (eyewear #105, waist-shape #106, lip-color #107, body-volume #96)** + 0 proposals; goal inputs 83.8% validated, `goal_unreachable: false` (floor 4001, `any_max_honest_floor_reached` 24/24).

**2026-08-11 03:10Z — GPU-STALL ROOT CAUSE SHIFT; EYEBROW #111 RESUME VERIFIED (measured, no registry change).** The original #118 blocker is GONE: `image_embed` (6.4 GB) has exited the 4090. nvidia-smi now shows ~1.75 GiB used / **~22.4 GiB free** (22368–22386 MiB) — but the scheduler converts to GiB (`MiB/1024` → 21.84–21.86) and compares against the 22.0 GB manifest claim, so `poll --gpu 4090` still returns **`vram_insufficient` by ~0.14 GiB (142–160 MiB)**. The residual occupant is the machine's DESKTOP graphics stack: gnome-shell 213 MiB, a Chromium renderer **1131 MiB**, steamwebhelper 39+63 MiB, Xwayland 8 MiB, alacritty 30 MiB (`stratum-prose-loo-4090-v1`, 12 GB, also queued at 01:02Z — another session's job, left untouched; FIFO keeps eyebrow first). **Eyebrow resume sequence verified end-to-end from THIS checkout:** branch `exp/stage-b-eyebrow-position-arm111-20260810` (HEAD 2e00632) is clean; `stage_b.py` sha = `ca00befd…` matches the manifest runner pin; pinned commit `cb8f7f23…` is an ancestor; `stage_b_launcher --manifest research/gpu-manifests/stage-b-eyebrow-position-v1.json --poll-and-launch` correctly takes the atomic `queued` path (no claim consumed; `request_if_missing=False` so the already-queued job is not re-requested). Review chain staged: `stage_b_review_launcher` (gemma4:e4b reviewer, present locally) derives its plan from the run root's records, so the review job (`--job-id stratum-stage-b-adversarial-review-eyebrow-position-v1`) must be requested only AFTER generation completes, then the parameterized wrapper polls it → tick-ready marker → `autonomous-tick --review-dir-from`. **An extended background poller (180 attempts × 60s) is running** and will atomically claim the moment display VRAM dips below 22528 MiB. **NEW FACT — queue expiry:** `queue_entry_expiry_h=24` runs on every poll; the eyebrow job (requested 2026-08-10 17:49:59Z) will be marked `expired` at **~2026-08-11 17:49:59Z** if still unclaimed — resume then becomes `stage_b_launcher --request` (re-queues same id) then `--poll-and-launch`. Validation on this branch: 825 tests passed; `validate-program` + `validate-dimension-registry` + `validate-gpu-manifest` + `validate-comparison-plan` all valid; label-sync `planned: []`; no tick-ready marker. Registry unchanged: **#111 eyebrow-position sole `research:active`**, 33 validated, 4 blocked, #112 jewelry proposal. #118 updated with the root-cause shift + expiry deadline + refreshed options ((a) amine_embed pause MOOT — already exited; (b) re-freeze to Strix unchanged; (c) accept wait now bounded by queue expiry).

**2026-08-10 (hand-gesture #109 ROUND-TRIP BETTER — harness-computed, cycle 33; eyebrow-position #111 active via exploit, selection_progress 34; 33 validated; body-volume #96 blocked with measured coverage numbers)**
