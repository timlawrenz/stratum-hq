...
**Last updated:** 2026-08-10 (decision boundary RESOLVED — #94 hair-texture re-scoped to CLIP zero-shot NEW MODEL CLASS + activated, #96 body-volume re-scoped to Anny SMPL-free + proposal, brainstorm-widen registered #105 eyewear / #106 waist-shape / #107 lip-color — all three measured DEGENERATE on the frozen cohort and blocked with numbers; 29 validated + 1 active + 3 blocked + 1 proposal)
**Phase / status:** ACTIVE — empirical Stage-B loop running; **29 validated** (27 goal feeders + goal arm dossier-context4k + reconstruction) + **1 active (hair-texture #94, CLIP zero-shot re-scope)** + **3 blocked (eyewear #105, waist-shape #106, lip-color #107 — all measured degenerate on the frozen cohort)** + **1 proposal (body-volume #96, Anny re-scope)**; goal inputs 93.1% validated, `goal_unreachable: false` (floor 4001, `any_max_honest_floor_reached` 24/24). The decision boundary is RESOLVED under the open-world sourcing directive (2026-08-05): model-class re-scopes are autonomous, not human rulings.
...
- **Registry** (`research/dimensions/evidence-dimension-registry-v1.json`): **29 validated** (body-type, clothing, hair, skin-color, lighting, dossier-context4k #36, setting #34, texture #35, reconstruction #37, vlm-dense-description #47, pose-articulation #62, pointmap-depth #58, matting-alpha #59, face-geometry #60, object-relations #61, scene-category #69, gaze-head-orientation #68, apparent-age #73, camera-viewing-angle #74, image-focus-depth-of-field #75, affordance-contact #76, iris-eye-color #80, facial-expression #81, hairstyle #82, body-configuration #83, face-visibility #84, environment-clearance #85, image-quality #95, garment-type #97),
  **1 active (hair-texture #94, CLIP zero-shot re-scope) + 3 blocked (eyewear #105, waist-shape #106, lip-color #107 — all measured degenerate) + 1 proposal (body-volume #96, Anny re-scope)**.
...
**Execute the active arm: hair-texture #94 (CLIP ViT-L/14 zero-shot re-scope, `research:active`).** The deterministic gradient axis measured DEGENERATE (2026-08-10 probe: 11/24 all one band, max_share 1.00; 13/24 abstains; 5 alternative deterministic discriminators non-separating). The re-scope to a NEW MODEL CLASS (open-weight CLIP ViT-L/14 zero-shot over the seg2 Hair crop, closed straight/wavy/curly/coily vocabulary + calibrated abstention floor) is authorized by the open-world sourcing directive (2026-08-05) and its capability probe is **NON-DEGENERATE** (24/24 measured, max_share 0.542, argmax conf median 0.632). Next: build the CLIP hair-texture module + tests, wire into stage_b (`evidence_kind == "hair-texture"`), freeze plan + manifest, GPU round-trip (4090 idle, scheduler-managed), independent review, `autonomous-tick`. Simultaneously, the three blocked brainstorm arms (#105/#106/#107) record small decisions on their issues (downgrade / different cohort / drop) — non-blocking for the loop. body-volume #96 (Anny re-scope) remains a proposal for a future selection.
...
- **Post-exhaustion brainstorm-widen (2026-08-06/07):** proposal arms **#58 point-map depth**, **#59 matting/alpha**,
  ...
  **Fourth widen (2026-08-08): #80 iris-eye-color, #81 facial-expression, #82 hairstyle,
  ...
  0.542) and ACTIVE; #96 body-volume re-scoped to Anny (non-SMPL) and back to proposal; sixth widen
  registered #105 eyewear, #106 waist-shape, #107 lip-color via the gated channel — all three capability-
  probed DEGENERATE on the frozen cohort and BLOCKED with measured numbers.**
