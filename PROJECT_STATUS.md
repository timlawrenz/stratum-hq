...
**Last updated:** 2026-08-10 (late cycle, GPU-stalled: waist-shape #106 v2 estimator built + probed → pre-registered gates FAIL, decision (a) executed; eyebrow-position #111 round-trip still queued behind #118 GPU conflict; jewelry #112 round-trip READY on PR #119; 861 tests green)
...
**Resolve the #118 GPU-routing ruling** (pause `image_embed` during Stage-B windows / re-freeze to Strix / accept wait — options on #118). Until then the 4090 cannot fit the 22GB claim (image_embed holds 6394 MiB; ~16.4GB free), so **no round-trip can run**: eyebrow-position #111 (job `stratum-stage-b-eyebrow-position-v1` queued since 2026-08-10 17:49:59Z) and jewelry #112 (frozen + launch-ready on PR #119) both wait on it. Resume sequence once VRAM frees: (1) `stage_b_launcher --manifest research/gpu-manifests/stage-b-eyebrow-position-v1.json --poll-and-launch`; (2) review wrapper with DISTINCT job id (`stratum-stage-b-adversarial-review-eyebrow-position-v1`) → tick-ready marker; (3) `autonomous-tick --review-dir-from <marker> --write`. Registry: #111 sole `research:active` (one-active invariant), #112 proposal, 33 validated, 4 blocked with measured numbers (#105/#106/#107/#96 — #106 now also carries the v2 estimator result: decision (a) executed, gates still FAIL).
