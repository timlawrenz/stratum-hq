...
**Last updated:** 2026-08-10 (arm #97 garment-type: strike-1 NOT_BETTER → honest absence-verbalization revision-2 → **BETTER**; hair-texture #94 now the active arm)
**Phase / status:** ACTIVE — empirical Stage-B loop running; **29 validated** (27 goal feeders + goal arm dossier-context4k + reconstruction) + **1 active (hair-texture #94)** + **1 proposal (body-volume #96)**; 0 blocked; goal inputs 89.7% validated, `goal_unreachable: false` (floor 4001, `any_max_honest_floor_reached` 24/24).
...
- **Registry** (`research/dimensions/evidence-dimension-registry-v1.json`): **29 validated** (body-type, clothing, hair, skin-color, lighting, dossier-context4k #36, setting #34, texture #35, reconstruction #37, vlm-dense-description #47, pose-articulation #62, pointmap-depth #58, matting-alpha #59, face-geometry #60, object-relations #61, scene-category #69, gaze-head-orientation #68, apparent-age #73, camera-viewing-angle #74, image-focus-depth-of-field #75, affordance-contact #76, iris-eye-color #80, facial-expression #81, hairstyle #82, body-configuration #83, face-visibility #84, environment-clearance #85, image-quality #95, garment-type #97),
  **1 active (hair-texture #94), 1 proposal (body-volume #96)**,
...
**Hair-texture (#94, `research:active`, NEW deterministic part from seg2 Hair region) is the sole `research:active` arm; its round-trip is NOT yet frozen — this is the next research target.** The deterministic selector picked hair-texture (exploit, EIG 0.55, novelty +0.15, tie-broken by id). Before a round-trip can launch, the arm needs: (1) capability + band-calibration probe on the frozen 24-item cohort (no band ≥ 75%, honest abstains); (2) frozen plan + GPU manifest (mirror the `freeze_*_manifest.py` / `stage_b_launcher` flow used by #97); (3) 96-record generation + independent review → `autonomous-tick`. The only remaining proposal is **body-volume #96** (NEW model class — open-weight whole-body mesh regression, needs the open-world capability probe before commitment). Sweep `exhausted: false`, next action none. Garment-type #97 is concluded (validated BETTER after strike-1 revision).
