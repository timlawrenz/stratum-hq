...
**Last updated:** 2026-08-11 00:35Z (GPU-stalled: waist-shape #106 v2 estimator probed → gates FAIL (decision (a) executed); eyebrow-position #111 round-trip still queued behind #118 GPU conflict; launcher freeze-pin verified — resume needs the eyebrow checkout, jewelry launchable from this one; 860 tests green)

**2026-08-11 00:35Z — GPU-STALL PERSISTS; LAUNCHER FREEZE-PIN VERIFIED (measured, no registry change).** Fresh ground truth: `image_embed` (pid 3301792, up 6h57) still holds 6394 MiB and serves 2 ESTAB connections from the crawlr ruby worker (pid 3304661); 4090 free ~15.9 GB < 22 GB claim → `stratum-stage-b-eyebrow-position-v1` remains queued (requested 2026-08-10T17:49:59Z, untouched). Attempted the documented resume step `stage_b_launcher --manifest research/gpu-manifests/stage-b-eyebrow-position-v1.json --poll-and-launch` from THIS checkout → failed **PRE-POLL**: `GPU manifest execution.runner_source_sha256 does not match the local runner`. Measured why: the manifest freezes `runner_source_sha256 ca00befd…` + `git_commit cb8f7f23…` (the eyebrow PR #117 pin); this checkout's `stage_b.py` is `f63a199b…` — which is exactly the **jewelry** manifest's runner pin (PR #119). Verified the eyebrow branch `exp/stage-b-eyebrow-position-arm111-20260810` (HEAD 2e00632) carries `stage_b.py ca00befd…` / `stage_b_launcher.py a05b0ea2…` matching the eyebrow pin, and its registry file == current registry (30cfbacd…) → **the eyebrow round-trip resumes from THAT checkout, not this one**. By contrast the CURRENT checkout can launch jewelry #112 directly once VRAM frees (runner hash matches its pin). Corrected resume sequence posted to #118. A failed pre-poll claim attempt is inert (no scheduler claim consumed; queue untouched). Validations: 860 tests passed; `validate-program` / `validate-dimension-registry` / `validate-gpu-manifest` (eyebrow + jewelry) / `validate-comparison-plan` (eyebrow) all valid; label-sync dry-run `planned: []`.

**2026-08-10 (late) — WAIST-SHAPE #106 v2 ESTIMATOR (decision (a)) BUILT + PROBED → GATES FAIL (measured).** While the round-trips stay GPU-queued behind the #118 ruling, the cycle executed the recorded autonomous option (a) on #106 ("develop a better waist estimator — limb-excluded torso profile"): new deterministic `research_harness/waist_shape.py` (v2) whose three pre-registered occlusion guards target v1's measured corruption (row-width minima over the torso span caught arm/hand pixels — v1: 10/24 measured, 4/24 plausible, max_share 1.00, median ratio 2.86, max 6.13): (a) arm-dominated rows (arm px ≥ 0.5×(strip+arm) px), (b) fragmented strips (run < 0.75×hull), (c) sliver guard (waist row ≥ 0.55×hip_w). Waist sub-band [y_s+0.40T, y_h−0.12T] (excludes the neck-row minima), hip sub-band [y_h−0.30T, y_h]; plane gate ≤45° + plausibility [0.7,2.4] follow `proportions` semantics (not weakened); sliver guard implies measured ratios are always within [1.0, ~1.82] (pinned by test). **Probe** (`waist-shape-v2-calibration-probe.json`, frozen 24-item cohort, CPU): the corruption is GONE — 0 implausible, plausible yield 7/7 vs v1's 4/10, ratios 1.21–1.71 anatomically sane (0unynv 3.13→1.56 with a clean 186px hip instead of the 903px artifact; 0mel7e 6.13→1.71; 03r1r 4.76→1.21) — **but the pre-registered gates FAIL: measured 7/24 < 10 floor AND band max_share 0.857 (hourglass 6 / moderate 1) ≥ 0.75 degeneracy gate.** 17 abstains = 7 low-conf + 6 plane-mix + 2 hip-occluded + 1 waist-occluded + 1 too-short; the axis does not discriminate on this portrait/crop-heavy cohort even with an honest estimator. Registry unchanged (waist-shape stays blocked; no state advance, one-active invariant holds); decision (b) different cohort (data authority) / (c) drop remain on #106 with the v2 measured numbers. 23 new tests (guards, band semantics, scale invariance, payload discipline). Branch `exp/waist-shape-v2-arm106-20260810`, draft PR; 861 tests + all validations green.

**2026-08-10 — JEWELRY #112 ROUND-TRIP READY (PR #119, GPU-stalled cycle).** The active arm's round-trip remains GPU-queued (see #118 below), so the cycle advanced the CPU-only pipeline: `research_harness/jewelry.py` (CLIP ViT-L/14 zero-shot over the seg2 Face_Neck crop — DOME-29 has NO Ear/Neck class, Face_Neck is the declared honest proxy; merged earrings-OR-necklace jewelry-present/no-jewelry band; earrings sub-band non-degenerate 0.739, necklace sub-band DEGENERATE 0.783 → payload-only; abstention floor 0.45 < probe min 0.5058; no-fabrication render wording both ways) + full `stage_b.py` evidence-kind wiring + frozen plan `stage-b-first500-jewelry-v1` (fingerprint 44e9c3ed…) + GPU manifest (96 records, 4090, git_commit pin 11a2ec7; `validate-comparison-plan` + `validate-gpu-manifest` valid) + 12 tests. Suite 837 passed, `validate-program` + `validate-dimension-registry` valid. Registry unchanged (jewelry proposal; #111 sole active — one-active invariant preserved). Also fixed a live hazard: a stray `stratum_review_poll_wrapper` (running since 16:54Z) used the GENERATION job id against an empty run root — it would have consumed the generation claim when VRAM freed; killed (held no claim), resume sequence documented on #118.
...
**2026-08-10 — BANGS-FOREHEAD #110 ROUND-TRIP BETTER (harness-computed, cycle 31).** Deterministic NEW-part arm (seg2 Hair over the pose2 eye-line-anchored forehead band; registered via the gated propose-dimensions channel this cycle). Capability/band probe PASS (22/24 measured, bangs 4 / partial-fringe 9 / swept-back 9, max_share 0.409). Frozen plan `stage-b-first500-bangs-forehead-v1` (96 = 24×4) generated on the 4090 via the scheduler (`stratum-stage-b-bangs-forehead-v1`, 96 records) + independent adversarial review (96 rows, `stage-b-bangs-forehead-v1-review`). `autonomous-tick` (marker-driven) computed **BETTER** — baseline 80 supported / 93 unsupported (ratio 0.4624) → evidence **210 supported / 12 unsupported (ratio 0.9459, Δ+0.4835), 17/22 paired positives, sign-test p=0.00845** (inconclusive:false). Registry: bangs-forehead → **validated** (cycle 31), **eye-openness #113 → active** (selected_via EXPLORE, ε-greedy slot, selection_progress 32). 783 tests + all validations green. Label-sync: #110 research:validated, #113 research:active.

**2026-08-10 — BODY-VOLUME #96 CAPABILITY-PROBED → BLOCKED (measured).** The arm's re-scoped ANNY stack was staged on owned hardware and the capability + model-qualification probe run on the 4090 via the scheduler: `research_harness/body_volume.py` + naver/multi-hmr Anny branch + `multiHMR_672_L_anny` checkpoint (1.53GB, sha256 9ad7497e…, free Anny checkpoint license — genuinely NO SMPL/SMPL-X). Model qualification PASS (state_dict 0/0, deterministic, detects 21/24 at calibrated det_thresh 0.1 vs 14/24 at the 0.3 default — threshold calibration disclosed). **Coverage gate FAIL:** on this portrait/crop-heavy cohort only 16/24 whole-body meshes clear height ≥0.8m, only 11/24 clear ≥1.2m (upper-body/close-up crops produced 0.50–0.96m meshes), and the norm_vol (volume/height³) band is CONFOUNDED WITH FRAMING (the 'fuller' class is exactly the shortest meshes). Measured < the pre-registered ≥18/24 floor → **body-volume marked BLOCKED**, decision recorded on #96: (a) relax the floor and accept a confounded band (not honest), (b) authorize a different full-body cohort (data authority), or (c) drop until a full-body cohort exists. Probe: `/mnt/nas-ai-models/research/stratum/body-volume-calibration-probe.json`. Label-sync: #96 → research:needs-human.

**2026-08-10 — REGISTRY TERMINAL → BRAINSTORM-NEW-DATA WIDEN (#109–#113).** With body-volume #96 blocked the selector reported `no actionable proposal — registry is terminal; run brainstorm-new-data` (30 validated / 4 blocked / 0 active / 0 proposals). Five genuinely-new candidates registered 2026-08-10 via the gated `propose-dimensions --require-new-evidence-part` channel, each naming a NEW evidence part or NEW model class measurable on the portrait/crop-heavy cohort (non-redundant with the 30 validated axes): **#109 hand-gesture** (NEW model class MediaPipe Hands 21-pt), **#110 bangs-forehead** (NEW deterministic part, seg2 Hair over the eye-line-anchored forehead band — VALIDATED BETTER this cycle), **#111 eyebrow-position** (NEW part, facemesh brow elevation), **#112 jewelry** (NEW part + CLIP zero-shot on ear/neck crops), **#113 eye-openness** (NEW part, eyelid/iris state — ACTIVE via the explore slot). Selector picked **bangs-forehead #110** (exploit, EIG 0.55, novelty +0.15, selection_progress 31); `autonomous-tick --write` activated it, and the round-trip concluded BETTER (cycle 31).

**2026-08-10 — HAIR-TEXTURE #94 CLIP RE-SCOPE ROUND-TRIP BETTER (harness-computed, PR #108 branch).** The deterministic gradient-orientation axis measured DEGENERATE (11/24 all one band, max_share 1.00; 13/24 abstains), so the arm was re-scoped to a NEW MODEL CLASS under the open-world sourcing directive: open-weight **CLIP ViT-L/14 zero-shot hair-texture classifier** over the seg2 DOME-29 Hair crop (closed straight/wavy/curly/coily vocabulary, calibrated abstention floor 0.35). Capability probe non-degenerate (24/24 measured, max_share 0.542; conf median 0.632 / min 0.373). Frozen plan `stage-b-first500-hair-texture-v1` (96 = 24×4) generated on the 4090 via the scheduler (`stratum-stage-b-hair-texture-v1`, 96 records) + independent adversarial review (96 rows, `stage-b-hair-texture-v1-review`). `autonomous-tick` computed **BETTER** — baseline 80 supported / 93 unsupported (ratio 0.4624) → evidence **124 supported / 47 unsupported (ratio 0.7251, Δ+0.2627), 17/23 paired positives, sign-test p=0.017345** (inconclusive:false). Registry: hair-texture → **validated** (cycle 27), **body-volume #96 → active** (exploit, selection_progress 30). 756 tests + all validations green. Label-sync: #94 research:validated, #96 research:active.

**2026-08-10 — DECISION BOUNDARY RESOLVED (strategist, PR #108, branch `exp/resolve-decision-boundary-arm94-hair-texture-20260810`).** The two blocked arms' gates were model-sourcing decisions covered by the standing open-world directive:
- **#94 hair-texture → re-scoped to NEW MODEL CLASS (CLIP ViT-L/14 zero-shot over seg2 Hair crop)** — deterministic gradient axis measured DEGENERATE (11/24 all one band, max_share 1.00; 13/24 abstains). Re-scope capability probe (hair-texture-clip-probe-20260810.json): **NON-DEGENERATE — 24/24 measured, straight 3 / wavy 13 / coily 6 / curly 2, max_share 0.542 < 0.75, argmax conf median 0.632 / min 0.373** → registry **active** (exploit, EIG 0.36, selection_progress 29).
- **#96 body-volume → re-scoped to Anny (non-SMPL whole-body volume regressor)** — naver/anny, Apache-2.0 code, CC0 MakeHuman assets, scan-free, genuinely SMPL-free. Option (b) of the license gate per the open-world directive; SMPL license path (option a) remains a human option but is NOT needed. Registry **proposal**.
- **Brainstorm-widen (selector mandate 'run brainstorm-new-data')** — 3 genuinely-new deterministic proposals registered via the gated `propose-dimensions --require-new-evidence-part`: **#105 eyewear** (seg2 Eyeglass class), **#106 waist-shape** (seg2 waist estimator, addresses the documented GOLIATH-308 waist gap), **#107 lip-color** (seg2 lip-region RGB). All three capability-probed on the frozen cohort and **measured DEGENERATE** (eyewear: 0/24 Eyeglass px + CLIP 23/23 'no glasses'; waist-shape: 4/24 plausible ratios all one band, median 2.86 outside human band; lip-color: 22/22 saturated natural-red, max_share 1.00) → all three **blocked** with measured numbers + decision notes on issues #105/#106/#107. Probe artifacts: `/mnt/nas-ai-models/research/stratum/{eyewear,lip-color,waist-shape}-calibration-probe.json` + `hair-texture-clip-probe-20260810.json`.
- Stale tick-ready marker (garment-type-v2, already concluded cycle 26) archived to prevent mishandling. 756 tests + all validations pass. Label-sync applied (#94 needs-human→active, #96 needs-human→proposal).

**2026-08-10 — Arm #97 garment-type: STRIKE-1 NOT_BETTER → HONEST REVISION → BETTER (harness-computed).** v1 round-trip (`stage-b-garment-type-v1`, 96 records + 96-row independent review) was harness-computed **NOT_BETTER** — baseline context-raw-no-evidence 80 supported/93 unsupported (ratio 0.4624) → evidence 173/29 (ratio 0.8564, Δ+0.394), but **sign-test p=0.10502 (inconclusive, 15/23 paired)** → strike 1/3 recorded, arm stayed sole-active. Root cause from the independent review's contradictions (7): the **caption model fabricated garments the evidence marks ABSENT** — "low-rise pink bottoms" on skin-dominant items, "pale blue button-down shirt" on lower-only, "black one-piece swimsuit" on upper-only — because the v1 rendering only said "lower body exposed"/"skin-dominant", leaving the measured `upper_garment_present`/`lower_garment_present` booleans payload-only. **Honest revision-2** (the arm-#95 band-degeneracy-recovery pattern): `_serialize_garment_type` + `render_garment_type` now **verbalize the measured absence explicitly** ("NO lower-body garment present", "NO upper-body garment present AND NO lower-body garment present") and add do-NOT-fabricate instructions; measurement/bands/floors/cohort/model/review protocol unchanged. v2 round-trip (`stage-b-garment-type-v2`, 96 + 96) → harness-computed **BETTER** — baseline 80/93 → evidence **194 supported / 9 unsupported (ratio 0.9557, Δ+0.4932), p=0.005311, 18/23 paired positives, contradictions 7→2**. Registry: garment-type → **validated** (cycle 26), **hair-texture #94 → active** (exploit, EIG 0.55, novelty +0.15, selection_progress 25), body-volume #96 remains a proposal. Label-sync confirmed (#97 `research:validated`, #94 `research:active`, #96 `research:proposal`). 744 tests + all validations green.
...
**Brainstorm-widen (2026-08-07, this cycle):** with the menu EXHAUSTED, 4 genuinely-New candidate arms were registered through the gated `propose-dimensions --require-new-evidence-part` channel (each names a NEW evidence part or NEW model class; redundant attribute-taggers rejected): **#73 apparent-age** (NEW model class MiVOLO-V2 age+gender transformer, Apache-2.0, arXiv 2307.04616), **#74 camera-viewing-angle** (deterministic camera-relative framing from seg2 + full-frame), **#75 image-focus-depth-of-field** (deterministic seg2-split acutance/DOF), **#76 affordance-contact** (deterministic subject-object contact from seg2+pose2). Selector (`autonomous-select`, exploit, EIG 0.65, novelty +0.15) picked **camera-viewing-angle #74**; `autonomous-tick` activated it (selection_progress 13). One-active invariant holds (17 validated + 1 active + 3 proposals).
...
**Resolve the #118 GPU-routing ruling** (pause `image_embed` during Stage-B windows / re-freeze to Strix / accept wait — options on #118). Until then the 4090 cannot fit the 22GB claim (image_embed holds 6394 MiB; ~15.9GB free), so **no round-trip can run**: eyebrow-position #111 (job `stratum-stage-b-eyebrow-position-v1` queued since 2026-08-10 17:49:59Z) and jewelry #112 (frozen + launch-ready on PR #119) both wait on it. Resume sequence once VRAM frees (freeze-pin corrected 2026-08-11 00:30Z — the launcher validates runner/launcher source hashes against the manifest pin): (1) **eyebrow #111**: `git checkout exp/stage-b-eyebrow-position-arm111-20260810` (pin cb8f7f23; the current checkout fails pre-poll on `runner_source_sha256`); (2) `stage_b_launcher --manifest research/gpu-manifests/stage-b-eyebrow-position-v1.json --poll-and-launch` (96 records → `/mnt/nas-ai-models/research/stratum/stage-b-eyebrow-position-v1`); (3) review wrapper with DISTINCT job id (`stratum-stage-b-adversarial-review-eyebrow-position-v1`) → tick-ready marker; (4) `autonomous-tick --review-dir-from <marker> --write`. Jewelry #112 is launchable from the CURRENT checkout (its runner pin f63a199b matches this `stage_b.py`). Registry: #111 sole `research:active` (one-active invariant), #112 proposal, 33 validated, 4 blocked with measured numbers (#105/#106/#107/#96 — #106 now also carries the v2 estimator result: decision (a) executed, gates still FAIL).
...
- **Post-exhaustion brainstorm-widen (2026-08-06/07):** proposal arms **#58 point-map depth**, **#59 matting/alpha**,
  ...
  **Fourth widen (2026-08-08): #80 iris-eye-color, #81 facial-expression, #82 hairstyle,
  ...
  recovery) → BETTER; #97 garment-type: strike-1 NOT_BETTER → honest absence-verbalization revision-2 →
  BETTER (2026-08-10, cycle 26). **2026-08-10 decision-boundary resolution (PR #108): #94 hair-texture
  re-scoped to CLIP ViT-L/14 zero-shot NEW MODEL CLASS (capability probe non-degenerate: 24/24, max_share
...
- #106 carries BOTH measured estimator outcomes: v1 (seg2 row-minima, corrupted by arm/hand px — 4/24 plausible, max_share 1.00) and **v2 decision-(a)** (limb-excluded torso profile `research_harness/waist_shape.py` + probe `waist-shape-v2-calibration-probe.json` — corruption eliminated, plausible 7/7, but measured 7/24 < 10 and max_share 0.857 ≥ 0.75 → gates FAIL; options (b) different cohort / (c) drop remain open).
