...
**Phase / status:** **ACTIVE — empirical Stage-B loop running autonomously.** Hold #18 is RELEASED. **#33 lighting is the sole `research:active`** arm in the issue tree (selector next pick). **Arm #31 (skin-color/tone) completed its full scheduler lifecycle run: VERDICT: BETTER** (support ratio 32.2%→89.3%, sign-test p≈0.000772); the registry advanced skin-color `active → validated`, and lighting became active. Prior validated arms: #4 baseline/parity (BETTER, PENDING_HUMAN_SPOT_CHECK advisory), #32 body-type (BETTER), #29 clothing (BETTER), #30 hair (BETTER). Stage A stays bounded and non-executing at `research/proposals/stage-a-caption-context-parity-preparation.md`.
...
- The dimension registry (`research/dimensions/evidence-dimension-registry-v1.json`) now marks body-type, clothing, hair, and skin-color validated; lighting is the sole active arm; setting/texture/dossier-context4k remain proposals (reconstruction #37 is a separate proposal issue). Sweep not exhausted. `autonomous-select` next picks lighting (#33).

## Immediate next action

Run the next selector-chosen arm (lighting #33) through the same frozen-cohort scheduler lifecycle once the determinism contract for lighting measurements (normal2 direction statistics + source luminance/histogram/dynamic-range) is wired into the Stage-B evidence-kinds path, then record its verdict and advance the registry. All runs are additive/noncanonical; no corpus mutation, no backfill, no legacy overwrite.
...
- #29–#37 are registered proposal arms; #32, #29, #30 hair, and #31 skin-color validated (all BETTER); #33 lighting is the next selector pick (now `research:active`).
...
**Arm #4: BETTER; Arm #32: BETTER; Arm #29: BETTER; Arm #30: BETTER; Arm #31: BETTER.** Declared deterministic evidence (geometry; body-type proportions; DOME-29 clothing coverage + dominant colors; hair region + color; exposed-skin tone) each significantly improves supported claims on the frozen 24-item cohort under fixed view/prompt/model/settings. The empirical verdicts await only the advisory human rubric spot-check for a formal PASS. Next: lighting (arm #33).
