...
**Last updated:** 2026-08-10 (arm #97 garment-type: strike-1 NOT_BETTER → honest absence-verbalization revision-2 → **BETTER**; hair-texture #94 blocked on model-class decision, body-volume #96 blocked on SMPL-X license ruling — 29 validated + 2 blocked + 0 active + 0 proposals)
**Phase / status:** ACTIVE — empirical Stage-B loop running; **29 validated** (27 goal feeders + goal arm dossier-context4k + reconstruction) + **0 active** + **2 blocked (hair-texture #94, body-volume #96)** + 0 proposals; goal inputs 89.7% validated, `goal_unreachable: false` (floor 4001, `any_max_honest_floor_reached` 24/24). Loop is at a decision boundary: the two remaining menu arms both sit behind human rulings.
...
- **Registry** (`research/dimensions/evidence-dimension-registry-v1.json`): **29 validated** (body-type, clothing, hair, skin-color, lighting, dossier-context4k #36, setting #34, texture #35, reconstruction #37, vlm-dense-description #47, pose-articulation #62, pointmap-depth #58, matting-alpha #59, face-geometry #60, object-relations #61, scene-category #69, gaze-head-orientation #68, apparent-age #73, camera-viewing-angle #74, image-focus-depth-of-field #75, affordance-contact #76, iris-eye-color #80, facial-expression #81, hairstyle #82, body-configuration #83, face-visibility #84, environment-clearance #85, image-quality #95, garment-type #97),
  **0 active + 2 blocked (hair-texture #94, body-volume #96) + 0 proposals**.
  `dimension-sweep-status`: `exhausted: false`, `next_action: none`
  (loop at a decision boundary until the two human rulings land), `goal_unreachable: false`
...
**The loop is at a decision boundary — 0 active arms; the two remaining menu arms both sit behind human rulings.** (1) **hair-texture #94 (`research:needs-human`):** the deterministic gradient-orientation specialist measured DEGENERATE on the frozen cohort (11/24 measured all one band, max_share 1.00; 13/24 honest abstains; 5 alternative deterministic discriminators non-separating — the axis is not cohort-degenerate, the signal is). Decision needed: authorize a NEW MODEL CLASS (learned/open-weight hair-texture specialist per the open-world directive) or downgrade the axis. (2) **body-volume #96 (`research:needs-human`):** open-world survey found HMR2.0/4D-Humans (MIT code) and PyMAF-X, but BOTH need the SMPL/SMPL-X model files (official SMPL site, non-commercial research license + registration). Decision needed: accept the SMPL/SMPL-X license + stage models locally, authorize a non-SMPL volume regressor, or downgrade. Both decisions are small and are recorded on issues #94/#96. Until a ruling lands, the selector has no arms to score; the validated goal arm (dossier-context4k, BETTER) and 28 other feeder dimensions remain in place, `goal_unreachable: false`.
