...
## Arm #60 — face-geometry evidence (NEW model class) — `[EMPIRICAL RUN COMPLETE — VERDICT: BETTER]`

**Date:** 2026-08-07
**Arm:** #60 — face-geometry (registered in the 2026-08-06 brainstorm-widen as NEW evidence part `face-geometry` + NEW model class `mediapipe-facemesh-3d`; was sole `research:active` arm, exploit, EIG 0.30, novelty +0.15; activated by the arm-59 tick as the successor arm)
**Code / PR:** `exp/stage-b-face-geometry-arm60-20260807` (draft PR #67, branch from the matting-alpha #66 lineage)
**Cohort:** frozen 24-item first-500 coverage-balanced subset (same manifest as all prior arms)
**Specialist (NEW model class):** `research_harness.face_geometry.compute_face_geometry` — open-weight **MediaPipe FaceLandmarker** (478-point 3D mesh, Apache-2.0, local CPU via the tasks API / XNNPACK; model `face_landmarker.task` sha256 64184e229b…, downloaded to the noncanonical model root, run on owned hardware only). Scale-invariant facial-geometry bands: eye spacing (interpupillary / face-width), mouth width / face-width, jaw width / face-width, and a plausibility-gated mid-face vertical share. Evidence input bound: `seg2.npy` (Face_Neck crop) + the already-decoded source RGB (pose2 stays validation-only). Only scale-invariant facts are verbalized; landmark coords + pixel bbox stay in the machine-readable `evidence_payload`.
**Capability probe (qualification gate step 2) — PASS, 21/24:** MediaPipe 1.0.0 has no legacy `solutions` (tasks API only; `ImageFormat` lives at `mediapipe.tasks.python.vision.core.image`); the detector is **resolution-sensitive and non-monotonic** on this cohort — crop-only 20/24, full-frame-only 12/24, **UNION 21/24** — so the module uses the union policy (full frame, then seg2 Face_Neck crop) and the crop slice must be `np.ascontiguousarray` (MediaPipe silently drops non-contiguous views). 3/24 abstain honestly (2 turned-head/no-face, 1 zero Face_Neck region). Bands calibrated from the measured cohort (eye 0.445/0.475 → 6/11/4 max 52%; mouth 0.333/0.400 → 6/10/5; jaw 0.783/0.830 → 7/11/3; midface 0.48/0.56 → 4/12/4); the mid-face vertical share carries a human-plausibility gate (one item measured an implausible 2.206 under tilt — payload-only, never verbalized).
**Run:** `/mnt/nas-ai-models/research/stratum/stage-b-face-geometry-v1` (96 captions, gemma3:27b digest a418f5838eaf, 4090 via scheduler; 4 conditions × 24) + independent adversarial review `/mnt/nas-ai-models/research/stratum/stage-b-face-geometry-v1-review` (96 rows, gemma4:e4b).
**Verdict (harness-computed `autonomous-tick --review-dir-from <marker> --write`):** **BETTER** — supported 47 → **155**, unsupported 99 → **36**; support ratio 0.3219 → 0.8115 (Δ +0.4896); paired positive 17/21; sign-test p = **0.003599**. `inconclusive: false`, `significant: true`. (3 of 24 items abstain on the face-geometry condition — the paired comparison uses the 21 measurable items.)
**Registry advance:** face-geometry `active → validated` (0 strikes); selector next → **object-relations #61** (active, selected_via exploit, selection_progress 10).
**Boundaries respected:** model downloaded as open weights and run on owned hardware only (no hosted third-party inference of the sensitive corpus); outputs only under the approved noncanonical research root; no `crawlr/approved` or `crawlr/stratum` mutation; no backfill; no legacy overwrite; the model cost is gated to the face-geometry plan only (`include_face_geometry` flag — other arms never pay it). The face-geometry dimension joined the dossier (`face-geometry:v1` evidence id + render_face_geometry + evidence_payload section).
**Validation:** 556 pytest passed; `validate-program` valid; `validate-dimension-registry` valid; `validate-gpu-manifest` valid. Verdict BETTER is empirical on this 24-item frozen cohort; a formal PASS still awaits the advisory human rubric spot-check.

## Arm #59 — matting / alpha-fidelity evidence — `[EMPIRICAL RUN COMPLETE — VERDICT: BETTER]`

**Date:** 2026-08-07
**Arm:** #59 — matting-alpha (registered in the 2026-08-06 brainstorm-widen; was sole `research:active` arm, exploit, EIG 0.39, novelty +0.15; activated by the arm-58 tick as the ε-greedy/successor arm)
**Code / PR:** `exp/stage-b-matting-alpha-arm59-20260807` (draft PR, branch from the pointmap-depth #65 lineage)
**Cohort:** frozen 24-item first-500 coverage-balanced subset (same manifest as all prior arms)
**Deterministic specialist:** `research_harness.matting_alpha.compute_matting_alpha` — scale-invariant alpha-fidelity facts from `matting.npy` (Sapiens2 per-pixel soft alpha matte, source-dimension-matched `(H,W)` float16, present 24/24 frozen items but unbound by any validated arm) + seg2 DOME-29 masks (Hair class): subject alpha-coverage band (sparse/centered/fills-frame from the alpha≥0.9 opaque fraction of the frame), boundary crispness band of the silhouette edge (soft/moderate/crisp from the median alpha-gradient over the 1-px silhouette ring), and soft-edge character (hair-dominant vs mixed vs skin-clean cutout from the share of the semi-transparent band that lies inside the Hair class — the "soft detachable hair strands" axis). Evidence inputs bound: `matting.npy` + `seg2.npy` (pose2 stays a validation-only read). Only scale-invariant facts are verbalized (ratios, normalized gradient descriptors, bands); absolute pixel areas/band widths and silhouette structure stay in the machine-readable `evidence_payload`. Genuinely-new evidence surface: per-pixel alpha edge fidelity no previously-validated arm binds.
**Hypothesis:** Declared matting/alpha-fidelity facts reduce unsupported edge-quality/hair/silhouette claims in captions vs matched no-evidence baseline.
**Calibration note (band probe on the real frozen cohort, before freeze):** first on-paper thresholds (soft-edge 0.015/0.030, detail 0.05/0.15, silhouette closedness) were DEGENERATE — soft_edge 21/24 "sharp", detail_band 23/24 "fine-detail", silhouette 24/24 "closed". Re-probed discriminators and rebuilt on the two genuinely-discriminating axes: boundary crispness (median ring alpha-gradient p50 spread → 11 crisp / 10 moderate / 3 soft, max 46%) and hair-soft share (6 skin-clean / 14 mixed / 4 hair-dominant, max 58%); coverage 5 sparse / 17 centered / 2 fills-frame (max 70.8%). No band ≥ 75% after recalibration (24/24 measurable, 0 abstentions). Silhouette closedness is honestly non-discriminating on this cohort (24/24 closed, no crops) and stays payload-only.
**Run:** `/mnt/nas-ai-models/research/stratum/stage-b-matting-alpha-v1` (96 captions, gemma3:27b A-fingerprint, 4090 via scheduler; 4 conditions × 24) + independent adversarial review `/mnt/nas-ai-models/research/stratum/stage-b-matting-alpha-v1-review` (96 rows, gemma4:e4b).
**Verdict (harness-computed `autonomous-tick --review-dir-from <marker> --write`):** **BETTER** — supported 47 → **187**, unsupported 99 → **29**; support ratio 0.3219 → 0.8657 (Δ +0.5438); paired positive 18/22; sign-test p = **0.002172**. `inconclusive: false`, `significant: true`.
**Registry advance:** matting-alpha `active → validated` (0 strikes); selector next → **face-geometry #60** (active, selected_via exploit, selection_progress 9).
**Boundaries respected:** local models only; outputs only under the approved noncanonical research root; no `crawlr/approved` or `crawlr/stratum` mutation; no backfill; no legacy overwrite; deterministic evidence computed in memory from existing `matting.npy`/`seg2.npy` only; scale-invariant verbalization retained (owner px→ratios/meters→payload rule). The matting-alpha dimension joined the dossier (`matting-alpha:v1` evidence id + render_matting_alpha + evidence_payload section) — honest evidence-density growth for the goal arm.
**Validation:** 547 pytest passed; `validate-program` valid; `validate-dimension-registry` valid; `validate-gpu-manifest` valid; label-sync applied (#59 → research:validated, #60 → research:active). Verdict BETTER is empirical on this 24-item frozen cohort; a formal PASS still awaits the advisory human rubric spot-check.
