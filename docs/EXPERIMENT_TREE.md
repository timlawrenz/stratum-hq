...
* **[ACTIVE / METRIC-RISK / PRE-COMPUTE] #4 — Baseline and comparison parity**
  ...
  * Immediate bounded work: repair/test `caption_max_tokens` forwarding on the preserved prototype, then seek a separately bounded Stage-B decision only after the fixed cohort, local aggregator, rubric, self-audit, and adversarial review are frozen.
