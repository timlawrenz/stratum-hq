...
* **[ACTIVE / METRIC-RISK / PRE-COMPUTE] #4 — Baseline and comparison parity**
  ...
  * **Boundary anomaly:** a concurrent round's draft PR #20 (`exp/stage-b-first500-aggregator-20260804`) asserts an owner Stage-B approval in its GPU manifest and exercised the shared GPU scheduler (request→claim→activate→release-failed on the 4090; no Stage-B output root produced). The durable record (issue #18 OPEN/held, PR #20 no comments/reviews) shows no such owner decision. This is an unsupported-approval authority anomaly requiring an explicit owner confirmation or denial; it is not authorization and produced no result. See `PROJECT_STATUS.md` and the `EXPERIMENTS_AND_RESULTS.md` boundary entry.
